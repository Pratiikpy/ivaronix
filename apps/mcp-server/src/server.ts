// Receipt verify path reads V3 → V2 → V1 (operator-funded V3 deployed; lookup matches studio/lib/chain.ts unifiedGetReceipt order). Reader-side fix shipped for §B-V2-37 (✅ shipped); anchor branch for slot 10+ MCP flows (doc_room_*, memory_consolidation) still queued — defensive writer-side covers when a slot 10+ extension lands.
