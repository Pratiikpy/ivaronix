// v3-lookup-allow: model writer emits slots 0-9 only (consensus + skill_exec). Reader-side fix shipped for §B-V2-37 (✅ shipped); writer stays grandfathered until/unless a slot 10+ extension lands here.
// Passport write path now V2-first via getActivePassportClient (K-6 memoryRoot-poisoning fix on V2). Closes the V1-only waiver from USER_TODO §B-V2-38 (✅ shipped).
