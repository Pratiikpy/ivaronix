/**
 * v17 · autonomous memory grant + revoke via /memory with fresh wallet.
 *
...
const OUT = resolve(REPO, 'QA_PROOF_PACK', 'submission-final', 'mm-prod-memory-grant-v17');
...
async function main(): Promise<void> {
  log(`v17 · memory grant + revoke autonomous`);
  ...
  try {
    ...
    writeFileSync(resolve(OUT, 'log.md'), `# v17 memory grant · ${new Date().toISOString()}\n\nFresh wallet: ${address}\n\n${events.map((e) => `- ${e}`).join('\n')}\n`);
