/**
 * v39 · Paid skill RUN via real MM — MM v13.30 confirm-btn locked in.
 *
 * Breakthrough from dump-mm-connect-popup.ts:
 *   - MM v13.30 popup uses data-testid="confirm-btn" universally for
 *     the primary action button (Connect / Sign / Switch / Confirm)
 *   - MM v13.30 popup also has data-testid="cancel-btn", accounts-tab,
 *     permissions-tab, edit — all of which v37/v38 text-matching might
 *     fuzzy-match into when .first() picks DOM-order earlier element
 *   - MM v13.30 uses LavaMoat scuttling: page.evaluate() against MM
 *     popups will fail; we rely on selectors + popup.content() only
 *
 * v39 strategy: try [data-testid="confirm-btn"]:not([disabled]) FIRST,
 * exclusively, with text-matching as a strict-fallback only after
 * confirm-btn has been polled for 5s with no match.
 */
...
const OUT = resolve(REPO, 'QA_PROOF_PACK', 'submission-final', 'mm-prod-paid-run-v39');
...
async function drivePopup(popup: Page, label: string, ms = 90_000): Promise<boolean> {
  ...
  while (Date.now() - start < ms) {
    ...
    // v39: MM v13.30 confirm-btn ONLY. No fuzzy text matching to avoid
    // accidentally clicking Cancel / Accounts-tab / Permissions-tab.
    const cta = popup.locator('button[data-testid="confirm-btn"]:not([disabled]), button[data-testid="page-container-footer-next"]:not([disabled]), button[data-testid="confirm-footer-button"]:not([disabled])').first();
...
async function paidRun(studio: Page, ctx: BrowserContext, extId: string): Promise<PaidRunResult> {
  log(`\n=== PAID RUN v39 · home → ensure mainnet → Use sample → Run → MM → /r/<id> ===`);
  try {
...
(async () => {
  log(`v39 paid-run real-MM · MM v13.30 confirm-btn locked · target: 1 fresh receipt`);
  log(`Studio: ${STUDIO}`);

  const tmpProfile = resolve(REPO, '.v39-profile');
