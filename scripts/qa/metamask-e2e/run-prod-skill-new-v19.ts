/**
 * v19 · /skill/new publish via fresh wallet.
 *
...
const OUT = resolve(REPO, 'QA_PROOF_PACK', 'submission-final', 'mm-prod-skill-new-v19');
...
async function main(): Promise<void> {
  log(`v19 · /skill/new manifest publish autonomous`);
  ...
  try {
    ...
    // Navigate /skill/new · creator publishes a new skill manifest
    const skillSlug = `qa-test-${Date.now().toString(36).slice(-6)}`;
    log(`\n=== /skill/new publish "${skillSlug}" ===`);

    // /skill/new likely auto-fires SIWE Sign popup on mount (same pattern as /memory)
    const knownPreSkill = new Set<Page>(ctx.pages());
    await studio.goto(`${STUDIO}/skill/new`, { waitUntil: 'domcontentloaded' });
    await studio.waitForTimeout(3_000);
    log(`  watching for auto-SIWE Sign popup`);
    const siwePopup = await pollPopup(ctx, extId, knownPreSkill, 20_000);
    if (siwePopup) { log(`  ✓ SIWE Sign popup detected · driving`); await drivePopupPatient(siwePopup, 'mm-siwe-sign', 60_000); }
    else log(`  ⚠ no SIWE popup in 20s (already-signed or page is open-access)`);
    await studio.bringToFront(); await studio.waitForTimeout(5_000);
    await snap(studio, 'studio-skill-new-loaded');

    // Find all text inputs + fill with safe defaults · then find Publish-like button
    log(`\n=== Step 1: Fill skill manifest form ===`);
    const inputs = await studio.locator('input[type="text"], input[type="number"], input:not([type]), textarea').all();
    ...
    for (const inp of inputs) {
      try {
        ...
        let val = `qa-test-${Date.now().toString(36).slice(-6)}`;
        const lower = (placeholder + name + id).toLowerCase();
        if (lower.includes('name') || lower.includes('slug')) val = skillSlug;
        else if (lower.includes('desc') || lower.includes('about')) val = `Autonomous test skill for v19 · ${new Date().toISOString()}`;
        else if (lower.includes('price') || lower.includes('og')) val = '0.001';
        else if (tag === 'textarea') val = `# qa test\n\nMinimal manifest body.\n\n## Behavior\n\nReturn input echo.`;
        await inp.fill(val).catch(() => {});
        filledCount++;
      } catch {}
    }
    log(`  filled ${filledCount} inputs`);
    await studio.waitForTimeout(1_500); await snap(studio, 'studio-skill-new-filled');

    log(`\n=== Step 2: Click Publish ===`);
    const publishBtnCandidates = [
    ...
    let publishBtn = null;
    for (let i = 0; i < 8; i++) {
      for (const sel of publishBtnCandidates) {
        const b = studio.locator(sel).first();
        if (await b.isVisible({ timeout: 1_500 }).catch(() => false)) { publishBtn = b; log(`  matched: ${sel}`); break; }
      }
      if (publishBtn) break;
      await studio.waitForTimeout(2_000);
    }
    if (!publishBtn) { log(`✗ no Publish/Submit/Save button found · /skill/new may render differently or require role-gating`); await snap(studio, 'studio-skill-new-no-publish'); await ctx.close(); return; }

    // Watch for /api/skill/save 200 (success) - Path 2 publish is OFF-CHAIN (sandbox FS)
    // Track responses: first Save fires 401 → triggers SIWE → after SIWE, app retries Save → 200
    let saveOk = false; let saveStatus = 0;
    studio.on('response', (resp) => {
      if (resp.url().includes('/api/skill/save')) {
        saveStatus = resp.status();
        if (resp.status() === 200) saveOk = true;
      }
    });

    const knownIssue = new Set<Page>(ctx.pages());
    await publishBtn.click({ timeout: 8_000, force: true }).catch(async () => { await publishBtn.dispatchEvent('click').catch(() => {}); });
    log(`  ✓ Save clicked (1st time — will likely 401, triggers SIWE)`);
    await snap(studio, 'after-publish-click');

    // Drive any auto-fired SIWE popup post-Save
    for (let popups = 0; popups < 3; popups++) {
      const popup = await pollPopup(ctx, extId, knownIssue, 20_000);
      if (popup) {
        log(`  popup ${popups} (likely SIWE Sign)`);
        await drivePopupPatient(popup, `mm-siwe-${popups}`, 60_000);
        knownIssue.clear();
    ...
    // Re-click Save after SIWE auth
    if (!saveOk) {
      log(`  re-clicking Save after SIWE (1st save status: ${saveStatus})`);
      const reSave = studio.locator('button:has-text("Save"):not([disabled]), button:has-text("Publish"):not([disabled]), button:has-text("Submit"):not([disabled])').first();
      if (await reSave.isVisible({ timeout: 5_000 }).catch(() => false)) {
        await reSave.click({ timeout: 6_000, force: true }).catch(async () => { await reSave.dispatchEvent('click').catch(() => {}); });
        log(`  ✓ Save re-clicked`);
        await studio.waitForTimeout(8_000);
      }
    }

    await snap(studio, 'studio-post-publish');

    // /skill/new is Path 2 (off-chain sandbox FS save) — success criterion is /api/skill/save 200, not nonce change
    log(`\n=== Verify publish via API ===`);
    log(`  final /api/skill/save status: ${saveStatus} · saveOk=${saveOk}`);
    const nonce1 = await provider.getTransactionCount(address);
    const bal1 = await provider.getBalance(address);
    log(`  Fresh wallet nonce: ${nonce1} (off-chain flow → 0 expected) · balance: ${formatEther(bal1)} OG`);

    if (saveOk) {
      log(`  ✓ /skill/new publish PROVEN · skill saved to per-wallet sandbox FS`);
      writeFileSync(resolve(OUT, 'final.json'), JSON.stringify({ creatorWallet: address, skillSlug, saveOk: true, saveStatus, flowType: 'path-2-off-chain', balanceFinal: formatEther(bal1) }, null, 2));
    } else {
      log(`  ⚠ /api/skill/save did not return 200 · final status: ${saveStatus}`);
      writeFileSync(resolve(OUT, 'final.json'), JSON.stringify({ creatorWallet: address, skillSlug, saveOk: false, saveStatus, balanceFinal: formatEther(bal1) }, null, 2));
    }
    await ctx.close(); return;
    ...
    writeFileSync(resolve(OUT, 'log.md'), `# v19 skill publish · ${new Date().toISOString()}\n\nFresh wallet: ${address}\n\n${events.map((e) => `- ${e}`).join('\n')}\n`);
