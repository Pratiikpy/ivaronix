...
- **`da retrieve` without `--out`** writes binary to stdout when stdout is a TTY. Will corrupt the terminal. **✅ FIXED 2026-05-14** · `apps/cli/src/commands/da.ts:119-138` now branches on `process.stdout.isTTY` — when TTY, prints size + sha256 instead of binary, exits 1, suggests `--out <path>` or piping to a binary-safe consumer. Pipe targets (file redirect, downstream command) still write binary unchanged.
...
### I-12 · `memory snapshot` prints sprint-tagged TODO; manifest is never persisted  *(severity A · ✅ FULLY CLOSED 2026-05-13 · B-V2-24 SHIPPED)*
- `apps/cli/src/commands/memory.ts` — pre-sweep the command only printed the manifest. Both halves now ship:
- **Storage half:** ✅ shipped sweep 201. `ivaronix memory snapshot --upload` writes the manifest JSON to 0G Storage and prints the storage rootHash + tx hash.
- **Chain half:** ✅ shipped iter-88. `ivaronix memory snapshot --upload --anchor-on-chain` resolves the signer's passport tokenId via `passportOf(signer)` (V2-first, V1 fallback) and calls `AgentPassportINFTV2.updateMemoryRoot(tokenId, storageRootHash)`. Real-chain proof: storage root `0x131403...` → updateMemoryRoot tx `0x2175670c90698610a799a4b3db1abcd60c4fc3aed3d89ee6017665e916201e90` · block 32967060 · gas 53300 · token #1 · 0x85e9dD63... (V2). Closes USER_TODO §B-V2-24.
...
### K-10 · `/api/dashboard/[addr]` filesystem walk reads arbitrary `.json` from disk  *(Medium · ✅ FIXED sweeps 177 + 196 + K-9 closure)*
- ✅ The K-10 attack vector — "any user who can write to operator's `.ivaronix/schedules`" — required K-9's anonymous-write surface to land a forged schedule. **K-9 closure** (245e017, SIWE + per-wallet sandbox + manifest Zod) removed that write path; only the operator's own filesystem owner can plant schedules now, and the operator-owns-operator-filesystem trust boundary is documented in `delegate.ts` threat-model JSDoc. **Sweep 177** redacted sensitive fields (`inputLabel`, `question`) from the public `/api/dashboard/<addr>` payload — even if a schedule surfaces, the user's prompt text + doc filename never reaches the caller. **Sweep 196** capped `loadSchedulesForOwner` at 50 entries to bound payload size. The walker is restricted to two specific candidate dirs (`.ivaronix/schedules` + `apps/cli/.ivaronix/schedules`), not an arbitrary path traversal. Per-claim filter: `s.ownerWallet.toLowerCase() === query.addr.toLowerCase()` keeps cross-wallet enumeration impossible. The original audit's "sign each schedule with the claimed wallet" fix would harden the operator-filesystem-compromise tier; that's K-21 territory (single operator key), queued for mainnet hardening, not a K-10 gap today.
...
### K-22 · `CapabilityRegistry.consumeRead` does not authenticate the grantee  *(High · ✅ FIXED 2026-05-12 · CapabilityRegistryV2 deployed at 0x1351CD87...3E1 · B-V2-15 SHIPPED)*
- **Status:** ✅ SHIPPED in `CapabilityRegistryV2` (commit 8e9bbcd + deploy commit + tx 0xdea605e6...). V2 adds `authorizedRelayers` mapping and gates `consumeRead` with `require(msg.sender == g.grantee || authorizedRelayers[msg.sender], ...)`. Operator wallet authorized as relayer post-deploy via tx 0xc7cf9fa9..., so the off-chain memory engine continues to work. V1 stays live at `0x3783f3c4834fCCBD553860e15c64C7E052646a8D` for any existing grants (chain history immutable); new grants land on V2. Mainnet redeploy waits on operator funding (USER_TODO §A-2).

### K-23 · `MemoryAccessLog` is permissionless — logs prove nothing  *(Medium · ✅ FIXED 2026-05-12 · MemoryAccessLogV2 deployed at 0xCbfE1f...96d · B-V2-16 SHIPPED)*
- **Status:** ✅ SHIPPED in `MemoryAccessLogV2` (tx `0x3d389299...`). V2 pins `CapabilityRegistryV2` (`0x1351CD87...`) at construction and enforces `logAccess` callers either via `msg.sender == agent` (self-log path) OR `CapabilityRegistryV2.isValid(grantId, msg.sender, scopeHash)` cross-check (grant-backed path). Random wallets revert. V1 stays live at `0xEe1aDFe76785377C4430B1325d86E58A6eC92119` for legacy events (chain history immutable); new writes land on V2. Off-chain consumers should still treat V1 events as informational; V2 events are authoritative. Mainnet redeploy waits on operator funding (USER_TODO §A-2).
...
### K-25 · `SubscriptionEscrow.cancel` lets agent grief client  *(Low · ✅ FIXED 2026-05-12 · SubscriptionEscrowV2 deployed at 0x74235b...B7F5 · B-V2-18 SHIPPED)*
- **Status:** ✅ SHIPPED in `SubscriptionEscrowV2` (tx `0x8e02e00c...`). V2 adds the `cancelGraceSeconds` window between agent-initiated cancel and EXPIRED, closing the griefing surface. ReceiptRegistryV3 pinned at construction so subscription_skill_exec anchors land on the canonical slot-9 receipt type. Mainnet redeploy waits on operator funding (USER_TODO §A-2). V1 stays live for any existing subscriptions (chain history immutable).
