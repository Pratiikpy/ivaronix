...
### J-3 · 14 `JSON.parse(readFileSync) as T` casts (disk-as-truth)  *(severity A · ⚠ PARTIALLY CLOSED sweep 205)*
- **Studio half:** ✅ closed. `apps/studio/src/lib/local-receipt.ts` now declares `LocalReceiptShape` (Zod schema requiring `id` + `type` with `.passthrough()` for the rest) and exports `safeReadReceiptBody(file)` which `safeParse`s before returning. Both Studio disk-read sites (`findLocalReceiptByRoot` + `loadAllLocalReceipts`) go through it. A migration-stale receipt JSON now returns `null` and the site is skipped instead of crashing `/r/[id]` first paint. `verify-studio-disk-receipt-safety.ts` regression locks the rule: no Studio file may write `JSON.parse(readFileSync) as ReceiptBody` outside the validator's own location.
- **CLI half:** still open. `apps/cli/src/lib/conversation.ts:57,63,74`, `apps/cli/src/commands/delegate.ts`, `apps/cli/src/commands/passport.ts` still cast disk JSON to typed shapes. The CLI is a single-user trust boundary (the operator's own files) so the blast-radius is smaller than Studio's public `/r/[id]` route, but the same shape lint should land before mainnet — queued in USER_TODO §B-V2-25.
