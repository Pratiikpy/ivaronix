...
### B-V2-2 · OG-image routes 503 on Vercel — `next/og` font asset doesn't resolve in the serverless bundle
- **Source:** plan-003 §A.5.5 + §B.2.3
- **Status (2026-05-11):** the three OG routes (`/opengraph-image`, `/0g/opengraph-image`, `/r/[id]/opengraph-image`) are DEPLOYED but return **HTTP 503 "OG image unavailable"** in production — a deliberate graceful degrade (was a 500 crash before; an empty fonts array would throw inside satori). Locally (`pnpm --filter @ivaronix/studio dev` and `next build` artifact inspection) the routes render. Vercel-only failure. Every other Studio surface is fully functional. Not a feature blocker — a social-card unfurl just shows no preview image; a judge clicking the link sees the full Studio.
- **Root cause:** `next/og`'s `ImageResponse` (satori) needs a TTF/OTF/WOFF font (NOT WOFF2), and `ImageResponse` has no usable bundled default font in the Vercel function. We vendored `apps/studio/src/lib/fonts/Outfit-SemiBold.ttf` and load it via `new URL('./fonts/Outfit-SemiBold.ttf', import.meta.url)` (the pattern `@vercel/og` itself uses for its default font — webpack emits the asset to `.next/server/chunks/static/media/` and `@vercel/nft` traces it into all three OG functions, verified in the `.nft.json`). But at runtime on Vercel, `fileURLToPath` on that URL throws `ERR_INVALID_ARG_TYPE` ("must be of type string or an instance of URL. Received an instance of URL") — webpack's URL-asset rewrite hands back a `URL` from a different module realm than `node:url`'s native `URL`, so the strict `instanceof` check fails. `fileURLToPath(url.href)` (the string form) didn't fix it either. `@vercel/og`'s own default font load (`fonts: undefined`) also throws "No fonts are loaded" — its `noto-sans-…ttf` reference isn't being statically traced into the function.
...
- **Remaining options to try (none quick):**
  1. `outputFileTracingIncludes` for `node_modules/**/next/dist/compiled/@vercel/og/*.ttf` (+ `yoga.wasm`/`resvg.wasm` if needed) on the three OG routes, then render with `fonts: undefined` so `@vercel/og` uses its bundled Noto Sans. Loses the brand Outfit font on the card but renders a real PNG.
  2. `outputFileTracingIncludes: { '/opengraph-image': ['src/lib/fonts/Outfit-SemiBold.ttf'], … }` + read it via `readFile(path.join(process.cwd(), 'src/lib/fonts/Outfit-SemiBold.ttf'))` instead of `import.meta.url` — but `process.cwd()` semantics on Vercel with `outputFileTracingRoot` set to the monorepo root need verifying first.
  3. Move the TTF into a tiny published/local `node_modules` shim package and `new URL('./Outfit.ttf', import.meta.url)` from there — `@vercel/og`'s pattern works precisely because its font is co-located in `node_modules` (webpack doesn't relocate it), so this is the closest like-for-like fix.
  4. Embed the TTF as a base64 string in `og-font.ts` (ugly, ~65 KB inline, but zero filesystem/URL resolution — guaranteed to work).
- **After it's fixed:** hit `https://ivaronix.vercel.app/r/<a-real-receipt-id>/opengraph-image` and confirm a 1200×630 PNG renders with receipt id + status chip + skill name. Preview check via `https://cards-dev.twitter.com/validator`. Capture a screenshot. Then close this entry with a `Closes audit B-V2-2` trailer.
