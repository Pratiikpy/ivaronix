...
### B-V2-45 · Edge-cache headers on public-manifest Studio routes (`docs/PRIVACY_NOTES.md §1` recommendation) · ✅ SHIPPED 2026-05-14 · this-iter
- **Source:** cron iter-170 driving plan §1059 (Privacy invariants walk). `docs/PRIVACY_NOTES.md §1 "Mitigation: edge-cache public manifests"` documents `cache-control: public, s-maxage=86400, stale-while-revalidate=604800` for routes that fetch public manifests. `apps/studio/next.config.ts:headers()` ships 4 security headers (X-Frame, X-CT, Referrer-Policy, HSTS) but NO `cache-control` directives. Privacy doc was promising a mitigation that's not in effect.
- **Why this matters:** combined with the read-proxy (B-V2-44), the edge cache means a typical reviewer hitting `/r/<id>` triggers zero indexer reads from the operator wallet on the warm path. Without the cache, every distinct viewer of a popular receipt triggers a fresh indexer sign-and-fetch that the operator signs.
- **Action:** (1) Identify the routes that fetch public manifests (`/r/[id]/page.tsx`, `/r/[id]/opengraph-image.tsx`, `/embed/r/[id]/page.tsx`, `/data-room/[id]/page.tsx`); (2) Either add `cache-control: public, s-maxage=86400, stale-while-revalidate=604800` to their Response headers, OR add a more specific entry in `next.config.ts:headers()` matching `/r/:id` + `/embed/r/:id` paths; (3) Verify with `curl -I <route>` that the header is set; (4) Lock with a `verify-studio-public-manifest-cache-headers.ts` regression.
- **Effort:** ~1h.
- **Closure:** Added 4 source-pattern Cache-Control headers in `apps/studio/next.config.ts:headers()` for `/r/:id`, `/r/:id/print`, `/embed/r/:id`, `/data-room/:id` — all with `public, s-maxage=86400, stale-while-revalidate=604800` per the privacy doc recommendation. Locked with `scripts/qa/metamask-e2e/verify-studio-public-manifest-cache-headers.ts` regression (4 assertions). Wired into the studio source-regression filter. Live HTTP `curl -I` verification queued for the studio-live filter (needs a running prod server).

### B-V2-44 · Read-proxy private key — runtime consumer (`docs/PRIVACY_NOTES.md §1` mitigation declared, no consumer)
- **Source:** cron iter-170 driving plan §1059 (Privacy invariants walk). `packages/runtime/src/env.ts:28,43,84,102` declares + parses `readProxyPrivateKey` from `IVARONIX_READ_PROXY_KEY` / `READ_PROXY_PRIVATE_KEY`. `docs/PRIVACY_NOTES.md §1` claims setting this env var makes public-fetch paths sign with this key instead of the operator's signer key. But no runtime path reads `env.readProxyPrivateKey` — verified by `grep -rnE "readProxyPrivateKey" apps/ packages/ scripts/ | grep -v node_modules` returning ONLY `env.ts` (the declaration site itself). The privacy mitigation is documented but the consumer never landed.
- **Why this matters:** the privacy doc reads "the operator wallet's address now only appears in indexer logs for actual writes (uploads, anchors). Reads sign with the read-proxy." That's a privacy promise. Today, setting `IVARONIX_READ_PROXY_KEY` in `.env` has zero runtime effect; the operator wallet still signs every indexer read.
- **Action:** (1) Identify the public-fetch path in `packages/og-storage/src/indexer.ts` and Studio `/r/[id]` data fetches; (2) Thread `env.readProxyPrivateKey` through to those paths, falling back to `env.signerKey` when unset; (3) Wire a regression `verify-read-proxy-consumer-exists.ts` asserting at least one runtime path reads `readProxyPrivateKey`; (4) Either confirm or update the privacy doc once the consumer ships. Until that lands, the doc is honestly marked PENDING (cron iter-170 fix).
- **Effort:** ~2-3h.
...
### B-V2-37 · V3-anchor-branch follow-up audit ✅ SHIPPED (readers migrated · writers stay defensive · 2026-05-14)
- **Source:** cron iter-120. `verify-v3-lookup-coexists-with-v2.ts` regression caught 10 files that look up `ReceiptRegistryV2` but not `ReceiptRegistryV3`.
- **Why this matters:** prevent V3 receipts from being invisible to indexers / telemetry / lookup paths, and prevent V2-only writers from reverting if extended to V3 slots.
- **Readers closed:** `serve.ts` (commit 9564463, iter-12), `apps/mcp-server/src/server.ts` (commit e300781, iter-10), `indexer.ts` (iter-23 · widened IndexerWorker to `1 | 2 | 3`, added V3 to registries[] array + db.totalV3 stat). V2 + V3 share the `ReceiptAnchored(id, root, agent, type, store, attest, relayer, nonce)` event ABI so a single worker handles both.
- **Writers:** 6 files (chat-v2, demo, doc, doc-bulk, model, pr) emit slots 0-9 only. The V2-only anchor is correct behavior today (no slot 10+ emission). The regression's `v3-lookup-allow:` markers grandfather the defensive-routing gap; if a future contributor extends any to slot 10+, they must add the V3 branch (pattern in `apps/cli/src/commands/receipt.ts` post-iter-120) before the regression accepts the change.
- **Intentionally V1 (allow-marked with reason):** `apps/cli/src/commands/debug.ts` (operator diagnostic surface for legacy V1+V2 inspection).
- **Commits:** 9564463 (serve), e300781 (mcp), iter-23 (indexer · this commit).
