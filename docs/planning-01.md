...
- **Receipt schema extended:** new types `doc_room_create` (slot 10) and `doc_room_read` (slot 11) added to `RECEIPT_TYPES` (`packages/core/src/types.ts`) and `ReceiptTypeSchema` (`packages/receipts/src/schema.ts`). **Chain encoding updated 2026-05-12** (B-V2-32 closure · ReceiptRegistryV3 deployed at `0x7396D536594e2BE833070c7EB441A10906046257`): new anchors on V3 record their canonical slot (10 / 11) on chain. V2-anchored receipts (pre-2026-05-12) carry the legacy coercion — `room create` → slot 5 (skill_exec) and `room read` → slot 4 (memory_access) — since chain history is immutable. CLI auto-selects V3 → V2 → V1 via nullish coalescing (`apps/cli/src/commands/room.ts`). Off-chain receipt body has always recorded the canonical name faithfully.
...
- **Voice check (§9):** zero banned words. One-clause sentences predominate. Real numbers replace adjectives ("1,165 receipts" not "extensively tested"; "$300/hour problem" not "expensive"). No "in today's fast-paced world" openers. <!-- wording-lint:allow:banned-phrase-example -->
...
- **Schema:** new `RECEIPT_TYPES.memory_consolidation = 12` in `packages/core/src/types.ts`; matching `'memory_consolidation'` in `ReceiptTypeSchema` (`packages/receipts/src/schema.ts`); new optional `request.priorReceiptIds: string[]` to record the lineage. Studio mirror in `apps/studio/src/lib/receipt-labels.ts`. **Chain encoding updated 2026-05-12** (B-V2-32 closure · ReceiptRegistryV3 deployed at `0x7396D536594e2BE833070c7EB441A10906046257`): new consolidations on V3 record the canonical slot 12 on chain. V2-anchored consolidations (pre-2026-05-12) carry the legacy coercion to slot 4 (memory_access) — chain history is immutable. CLI auto-selects V3 → V2 → V1 via nullish coalescing (`apps/cli/src/commands/passport-consolidate.ts`). Same Phase A → V3 progression we used for doc_room_*.
