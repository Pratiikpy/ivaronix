...
| Receipts | V3-first read fallback (V3->V2->V1) | 0 | Open `/r/<id-anchored-on-V1>` (e.g. fixture id 8). | `ivaronix receipt verify tests/fixtures/anchored-receipts/v1-anchored-id-8.json` | Studio reads ReceiptRegistryV3 first (slots 10/11/12), falls back to V2 (slots 0-9), then V1 legacy. V1, V2, and V3 receipts all resolve on the same `/r/<id>` route per iter-114/iter-119 V3-blindness fixes. | Screenshot of V1 + V2 + V3 receipts rendering. |
...
| npx bundle | `npx ivaronix` install + verify | 0 | Not needed. | On a clean shell (NOT inside the repo): `npx ivaronix receipt verify <known-id>`. | Bundle downloads, runs, returns FULLY VERIFIED ✓ or honest reason. This is the README's quickstart and what a judge types. Bundle source: `apps/npx-cli/dist/ivaronix.mjs` (built artifact, ~8 MB esbuild, gitignored — exists only after `pnpm --filter @ivaronix/npx-cli build`). <!-- qa-plan-path-allow:built-artifact --> | Terminal recording from a non-repo shell. |
...
| `version` | Today: `z.literal('1.0')` (hardcoded in `packages/receipts/src/builder.ts:56`). All receipts — anchored on `ReceiptRegistry` V1 / V2 / V3 — carry `version: '1.0'`. The "V2/V3" in the registry contract names refers to the on-chain CONTRACT version, not the receipt-body schema version. JCS+keccak hash path (per `packages/core/src/jcs.ts` and `docs/HASH_FUNCTION.md`) is selected separately, not by this field. Plan claim "V2 receipts carry 2.0+" was conflating these two concepts. | Every produced receipt has `version: '1.0'` exactly. Fixture `v1-anchored-id-8.json` and live anchored receipts both carry `'1.0'`. |
...
| `chainAnchor.registryAddress` | One of the known `ReceiptRegistry` V1 / V2 / V3 addresses (V3 admits slots 10/11/12). | Cross-check vs `KNOWN_RECEIPT_REGISTRIES` set in `@ivaronix/core` (currently 3 testnet entries: V1 `0x97376C6f...`, V2 `0xf675d418...`, V3 `0x7396D536...`). |
...
## Source-File Regression Suite (<!-- regressions:auto:total -->115<!-- /regressions:auto:total --> verify-\*.ts files on disk · <!-- regressions:auto:automated -->96<!-- /regressions:auto:automated --> automated · <!-- regressions:auto:live -->19<!-- /regressions:auto:live --> require live server)

`scripts/qa/metamask-e2e/verify-*.ts` ships **<!-- regressions:auto:total -->115<!-- /regressions:auto:total --> verify-\*.ts files** (counted via `find scripts/qa/metamask-e2e -maxdepth 1 -name 'verify-*.ts' | wc -l`). Of these, **<!-- regressions:auto:automated -->96<!-- /regressions:auto:automated --> run automatically** (pre-commit on Studio offline · CI on all three offline filters); the remaining **<!-- regressions:auto:live -->19<!-- /regressions:auto:live --> require a running Studio dev server** and are gated behind the `studio-live` filter. The `verify-no-orphan-regressions.ts` meta-regression confirms every file is wired to at least one filter. A serious tester re-runs the whole offline suite to confirm what was tested matches what's on disk.

| Suite | Command | Pass condition |
|---|---|---|
| Studio offline regressions (<!-- regressions:auto:studio -->77<!-- /regressions:auto:studio -->) | `pnpm --filter qa-metamask-e2e run regressions:studio` | All <!-- regressions:auto:studio -->77<!-- /regressions:auto:studio --> PASS. Pre-commit runs this suite on every `git commit`. |
| CLI regressions (<!-- regressions:auto:cli -->14<!-- /regressions:auto:cli -->) | `pnpm --filter qa-metamask-e2e run regressions:cli` | All <!-- regressions:auto:cli -->14<!-- /regressions:auto:cli --> PASS. |
| Contract regressions (<!-- regressions:auto:contracts -->5<!-- /regressions:auto:contracts -->) | `pnpm --filter qa-metamask-e2e run regressions:contracts` | All <!-- regressions:auto:contracts -->5<!-- /regressions:auto:contracts --> PASS. |
...
|---|---|---|---|---|
| 0G Chain | `packages/og-chain/` + 3 ReceiptRegistry versions (V1/V2/V3) + AgentPassportINFT V1/V2 + CapabilityRegistry V1/V2 + MemoryAccessLog V1/V2 + SkillRegistry V1/V2 + SubscriptionEscrowV2 + Erc7857Verifier deployed at the addresses in `contracts/deployments/testnet.json` | Anchor a receipt + read it back via V3-first → V2 → V1 fallback (slots 10/11/12 require V3 per B-V2-32). Open every contract on chainscan-galileo.0g.ai. | All <!-- numbers:auto:contracts.deployed -->13<!-- /numbers:auto:contracts.deployed --> contracts deployed, readable, and used by Studio/CLI. | Chainscan screenshots + UI ↔ CLI tx hash match. |
...
| MCP server (5 tools) | `apps/mcp-server/src/server.ts` — actual names are snake_case per MCP convention: `ivaronix_ask`, `ivaronix_verify_receipt`, `ivaronix_search_memory`, `ivaronix_install_skill`, `ivaronix_passport_show`. (Plan originally documented dot-notation; corrected iter-25.) | `pnpm --filter @ivaronix/mcp-server dev` boots the stdio server. Driving iter-25: `{"jsonrpc":"2.0","id":1,"method":"tools/list"}` over stdin returns all 5 tools with valid JSON-Schema input schemas. | All 5 tool names appear with JSON-Schema inputs · required fields enforced. Full Claude Desktop integration stays `BLOCKED`-with-reason for the live phone-side. |
...
| Test | Action | Pass condition |
|---|---|---|
| v1 receipt resolves on the live `/r/<id>` | Fixture `tests/fixtures/anchored-receipts/v1-anchored-id-8.json` (V1, schemaVersion 1.x) | Studio reads V3 first → V2 → V1 fallback per iter-119 destructure regression. Page renders correctly with VERIFIED chip. |
...
|---|---|
| Studio CSP | `apps/studio/next.config.ts:114-117` says CSP "deliberately omitted — needs end-to-end app testing to draft policy that allows wagmi + Next.js inline scripts". Tracked in `docs/USER_TODO.md §B-V2-42`. Mark `PENDING` until CSP ships. |
...
| `ivaronix indexer ...` | 0 | Indexer reads (receipt-by-agent, receipt-by-root, count). | Same data as the V3-first chain client + Studio dashboard (V3→V2→V1 fallback per iter-120 + iter-125 cascade). |
...
| 30s fresh receipt | `pnpm ivaronix demo` (after env config) | Anchors a real receipt in ~3-5s; prints proof URL + chainscan + verify command. README claim "30 seconds" — tester records actual time. |
...
| Item | State | Test |
|---|---|---|
| Sentry DSN slot | `SENTRY_DSN` env var documented in `.env.production.template` (commented out) — actual SDK wiring NOT implemented in Studio source yet. | `grep -ri 'sentry' apps/studio/src/` returns 1 hit today: `apps/studio/src/lib/error-sanitize.ts:36` regex pattern `\b(?:IVARONIX|OG|EVM|ZG|NVIDIA|SENTRY|UPSTASH)_[A-Z0-9_]+\b` that strips env-var leaks from error messages — a false positive, not actual Sentry wiring. No `@sentry/*` package imports. Mark `PENDING` against `§B-V2-26` (production error capture). When wired, test: a deliberate runtime error in `/api/run` produces an entry in the Sentry dashboard within 60s. |
