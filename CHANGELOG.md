...
- Studio `/r/<id>` for the 3 flagship mainnet receipts (66, 68, 70) showed "Anchored on chain. The body JSON is needed to re-derive the hash + signature locally — request it from the operator or run `ivaronix receipt show` on a machine that has the cache." A stranger judge landing on /r/66 saw the chain anchor and skill metadata but no AI output. `apps/studio/src/lib/local-receipt.ts` already supported a bundled-body path (7 testnet receipts were bundled at `apps/studio/src/data/receipts/anchored/`); zero mainnet receipts were bundled. Copied the 3 CLI fixture bodies into the Studio bundle so `findLocalReceiptByRoot` resolves them server-side. Production /r/66 now renders the actual AI output ("Most concerning clause due to extreme financial exposure and zero lease stability. Evidence: Section 4 allows the landlord to unilaterally raise rent by up to 50% per quarter…") with caption "Full body matches on-chain root". Bug-15 closure.
- Studio `/r/<id>` for receipts NOT in the curated bundle (e.g. /r/72, the most recent anchor) showed "request it from the operator or run `ivaronix receipt show` on a machine that has the cache" — gate-keepy framing for receipts a judge clicked from the live feed. Replaced with accurate, honest copy: "Anchored on chain. Run `ivaronix receipt show <id> --network <net>` to read the on-chain anchor metadata (no credentials needed). Full body re-verify requires a 0G Storage download — any wallet works for read." Verified on `/r/72`. Bug-16 closure.
- Studio `/r/1004` returned HTTP 404 on mainnet production because 1004 is a Galileo testnet receipt id and Studio reads from the active network's `ReceiptRegistry`. The footer's "Sample receipt" link (visible on EVERY page) + 5 home-page CTAs (Verify · Share · Proof Explorer · Embed iframe + href) + 4 secondary surfaces (/faq, /learn, /thesis, /0g, /docs snippets) all sent judges to a 404. Introduced `apps/studio/src/lib/sample-receipt.ts` as the single source of truth: `getSampleReceiptId()` returns `66` on mainnet (bundled fixture · quick-tier private-doc-review), `1004` on testnet. All 7 affected files refactored to use the helper. Studio typecheck clean, 76/76 source-file regressions PASS, production build clean. Bug-17 closure.
