...
| 8 | `8255f5a` | `deriveRiskLevel` heuristic missed plain-text "Risk Level: high" trailer · 5 legal-cluster receipts reported riskLevel=low despite identifying $4,800 non-refundable deposits + tenant indemnification clauses | Added PLAIN_HIGH/MED/LOW regex patterns matching the prompt-instructed trailer (with optional **bold** markdown) · 4 new tests, 44/44 risk-suite pass · **PRODUCTION-VALIDATED CROSS-SKILL**: receipt 79 (private-doc-review · `rcpt_01KRP8P4Q7NWKQ4VWF23BKENSD` · tx `0x701de937...`) → riskLevel=high via PLAIN_HIGH_RX matching "Risk Level: high" trailer; receipt 80 (nda-triage-reviewer · `rcpt_01KRP9B2QP3HPYNRV85Z2S4PW4` · tx `0xb53f6ac2...`) → riskLevel=high via JSON_REFUSE_RX matching `"signature_recommendation": "escalate"`. Both regex patterns from the fix firing on production-anchored receipts. Compare with legacy receipts 1-5 which still permanently say "low" on chain |
