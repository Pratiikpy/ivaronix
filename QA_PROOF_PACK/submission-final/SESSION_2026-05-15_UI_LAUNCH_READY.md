...
| 8 | `8255f5a` | `deriveRiskLevel` heuristic missed plain-text "Risk Level: high" trailer · 5 legal-cluster receipts reported riskLevel=low despite identifying $4,800 non-refundable deposits + tenant indemnification clauses | Added PLAIN_HIGH/MED/LOW regex patterns matching the prompt-instructed trailer (with optional **bold** markdown) · 4 new tests, 44/44 risk-suite pass · **PRODUCTION-VALIDATED ON 4 SKILLS**: V2 testnet receipt 79 (private-doc-review · PLAIN_HIGH_RX trailer → high), V2 testnet receipt 80 (nda-triage-reviewer · JSON_REFUSE_RX "escalate" → high), V3 testnet receipt 9 (contract-renewal-clause-detector → high), V3 testnet receipt 10 (term-sheet-risk-scanner aggressive Series B → **medium** — proves heuristic differentiates, not always "high"). 5th skill (legal-citation-verifier) hit Router 429 rate limit after 4 back-to-back paid runs · retries on next cron iteration. The deriveRiskLevel fix correctly reads model output across legal cluster shapes. Legacy receipts 1-5 still permanently say "low" on chain |
