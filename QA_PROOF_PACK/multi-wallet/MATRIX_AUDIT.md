...
| 6 | 825 | Data room share | PENDING (chain complete + TEE-independent re-verify N/A) | ✅ iter-134 — room `01KRFBG2XC8G20JDJ81CD614AX`, create tx `0xb0f3f496...` block 33009656 (V3 slot 10), Wallet B read tx `0xeb3bce64...` block 33009843 (V3 slot 11 id=6). ✅ iter-137 — `receipt verify --tee-independent` returns "ANCHORED (TEE-independent N/A)" — doc_room_read has no inference so TEE re-verify is correctly skipped; schema/hash/signature/anchor all PASS. | ❌ no /data-room/<id> capture from Wallet B's MM | Bug found + fixed iter-134: room.ts:486 read path was V1-only while write side was V2-first since iter-122. UI gate on real MM connection. |
...
| 12 | 863 | Marketplace: buyer runs creator skill | **PENDING (chain + CLI + public-Studio surfaces all consistent · MM-connected UI still gated)** | ✅ iter-136 — Wallet B ran private-doc-review · receipt `rcpt_01KRFC89GHCCKSFTJN593JSJF8` V2 id=10 (tx `0xea24bab7...` block 33011215). ✅ iter-137 — receipt FULLY VERIFIED ✓ via --tee-independent. ✅ iter-138 — CLI `receipt show 10` returns agent=Wallet B; CLI `receipt list --agent 0xaf29...` finds 3 Wallet B receipts (#11+#10+#6) merged across V1+V2+V3 registries; **production Studio `/r/10` HTTP 200 + HTML contains Wallet B's address `0xaf295d3c842bc1145E818d7FEf2c929726625620`** — chain→CLI→Studio public surface data flow end-to-end consistent. | ❌ Wallet B NOT imported into MM + connected to Studio interactively | Three surfaces (chain · CLI · public Studio render) all show Wallet B as agent. The remaining UI gap per the codified rules is specifically "Wallet B imported into MetaMask + used through Studio" — i.e. the connected-wallet interactive flow, not the read-only public render. |

### 3-wallet rows

| # | Plan line | Feature | State | Chain proof | UI/MM proof | Notes |
|---:|---:|---|:---:|:---:|:---:|---|
| 13 | 757 | Fee-split variation honoring manifest | **PENDING (chain complete + BOTH receipts FULLY VERIFIED)** | ✅ iter-136 — TWO receipts from Wallet B in same session: `rcpt_01KRFC89...` (private-doc-review 90/10 honored: declaredCreatorBps=9000) and `rcpt_01KRFCB8...` V2 id=11 (content-pitch-review 70/30 honored: declaredCreatorBps=7000). Default 90/10 did NOT bleed into the 70/30 skill — each receipt records the EXACT split from its own manifest. ✅ iter-137 — BOTH receipts verify **FULLY VERIFIED ✓** with --tee-independent. | ❌ no Studio side-by-side capture | Two skill manifests' fee-split blocks correctly enforced per-receipt + both receipts pass gold-standard TEE-independent verification. TIER_2 multiplier (8500bps) applied uniformly to both; declared bps preserved verbatim per skill. |
